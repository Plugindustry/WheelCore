...
public class EntitySelector {
	...
	private void createPredicates(Map<String, String> predicateStringPair, List<Predicate<Entity>> allPredicates) throws SelectorSyntaxException {
		...
		if (predicateStringPair.containsKey("distance")) {
			if (predicateStringPair.containsKey("dx") || predicateStringPair.containsKey("dy") || predicateStringPair.containsKey("dz")) {
		...
		if (predicateStringPair.containsKey("dx") && predicateStringPair.containsKey("dy") && predicateStringPair.containsKey("dz")) {
			...
			allPredicates.add((entity) -> {
				...
				return Math.abs(dx) > Math.abs(entityVec.getX()) && Math.abs(dy) > Math.abs(entityVec.getY()) && Math.abs(dz) > Math.abs(entityVec.getZ());
			});
		}
		...
		if (predicateStringPair.containsKey("x_rotation")) {
			...
			allPredicates.add((entity) -> entity.getLocation().getPitch() >= rotationXMin && entity.getLocation().getPitch() <= rotationXMax);
		}
		...
		for (String scorePredicate : scorePredicates) {
			...
			try {
				...
				predicates.add((entity) -> {
					...
					return o.getScore(getEntityName(entity)).getScore() > finalScoreMinVal && o.getScore(getEntityName(entity)).getScore() < finalScoreMaxVal;
				});
			}
		}
	...
	private List<Entity> getEntities(Sort sortMethod) {
		...
		switch (sortMethod) {
			case NEAREST:
				...
				break;
			case FURTHEST:
				e.sort((en1, en2) -> (int) Math.ceil(baseLoc.distance(en2.getLocation()) - baseLoc.distance(en1.getLocation())));
				break;
			case RANDOM:
