...
public class PowerManager {
    ...
    public static void inputPower(Location block, double power) {
        if (!(MainManager.getBlockInstance(block) instanceof EnergyInputable))
            throw new IllegalArgumentException("Not an energy inputable block");
        inputMap.put(block, new InputRequest(power));
    }

    public static void outputPower(Location block, double power) {
        if (!(MainManager.getBlockInstance(block) instanceof EnergyOutputable))
            throw new IllegalArgumentException("Not an energy outputable block");
        outputMap.put(block, new OutputRequest(power));
    }

    public static void onTick() {
        // Filter
        inputMap.keySet().removeIf(location -> !(MainManager.getBlockInstance(location) instanceof EnergyInputable));
        outputMap.keySet().removeIf(location -> !(MainManager.getBlockInstance(location) instanceof EnergyOutputable));
        ...
        // Input and spread
        MainManager.getBlockMapping().values().stream().filter(base -> base instanceof Wire)
                .map(MainManager.blockDataProvider::blocksOf).flatMap(Collection::stream).forEach(location -> {
                    ...
                    HashSet<Location> inputs = Stream.of(location.clone().add(1, 0, 0), location.clone().add(-1, 0, 0),
                                    location.clone().add(0, 1, 0), location.clone().add(0, -1, 0), location.clone().add(0, 0, 1),
                                    location.clone().add(0, 0, -1)).filter(inputMap::containsKey)
                        ...
                        if (MainManager.getBlockInstance(packet.src) instanceof EnergyOutputable &&
                            ...
                            if (!inputs.isEmpty() && r.nextBoolean()) {
                                Location availableInput = StreamUtil.randomPick(
                                        inputs.stream().filter(loc -> !loc.equals(packet.from))).orElse(null);
                                if (availableInput != null) {
                                    ...
                                    } else inputRequest.result += packetClone.amount;
                            ...
                            // Spread
                            Optional<Location> optional = StreamUtil.randomPick(
                                    BlockUtil.findWireAround(location).filter(loc -> !loc.equals(packet.from)).filter(loc -> {
                                        ...
                                        return Objects.requireNonNull(instance).getMaxTransmissionEnergy() > data1.statNext;
                                    }));
                            ...
                            if (optional.isPresent()) {
                                ...
                                packetClone.amount = max < current + packetClone.amount ? max - current : packetClone.amount;
        ...
        // Swap
        MainManager.getBlockMapping().values().stream().filter(base -> base instanceof Wire)
                .map(MainManager.blockDataProvider::blocksOf).flatMap(Collection::stream).map(MainManager::getBlockData)
                .forEach(data -> {
    ...
    public static class InputRequest extends Request {
        ...
        public boolean equals(final Object o) {
            ...
            if (!(o instanceof final InputRequest other)) return false;
            if (!other.canEqual(this)) return false;
            if (Double.compare(this.power, other.power) != 0) return false;
    ...
    public static class OutputRequest extends Request {
        ...
        public boolean equals(final Object o) {
            if (o == this) return true;
            if (!(o instanceof final OutputRequest other)) return false;
            if (!other.canEqual(this)) return false;
            if (Double.compare(this.power, other.power) != 0) return false;
            return Double.compare(this.result, other.result) == 0;
        }
